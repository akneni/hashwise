...
__device__ int update_str(char* target, int len_target, char* elements, int len_elements) {
    // Returns 1 if updated sucessfully, returns 0 if the end of the permutation has been reached, returns -1 if something went wrong
...
__global__ void global_sha256_brute_force(int* len_chars_ptr, char* elements, int* len_elements_ptr, ull* part, const char* target, char* res){
    ...
    while (start < end){
        ...
        if (cudaStrcmp(checker, target, 64) == 0){
            printf("Changing res from thread <%d>\n", threadID);
            cudaStrcpy(res, current, len_chars);
            break;
        }
...
int main(){
    ...
    // char* hash = "406e409a273a55a1d66938548120ebad2cfc5600e43bf8f3a1f0294e4ce4bf26";
    // char* elements = "0123456789";

    // char* res = sha256_brute_force(6, elements, 10, hash, 600, 32);
